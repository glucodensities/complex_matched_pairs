...
// [[Rcpp::export]]
double energypairedwildboostrap2(arma::mat X1, arma::mat Y1, arma::mat X2, arma::mat Y2,arma::umat permutaciones, int ker_type){
  ...
  arma::uvec contador1(n2);
  arma::uvec contador2(n3);
  int contar=-1;
  ...
  arma::mat d21 =  (*kernel_Y).getKernelMatrix(X2,X2);
  arma::mat d22 =  (*kernel_Y).getKernelMatrix(Y2,Y2);
  arma::mat d23 =  (*kernel_Y).getKernelMatrix(X2,Y2);
  ...
  double salidafinal2 = acum2+acum3-acum4;
  double estadisticofinal = acum*0.5+0.5*salidafinal2;
  double resfinal = estadisticofinal;

//  double resfinal= acum;
  double ln= log(n1);
  int npermutaciones=permutaciones.n_rows;
  ...
  for(int r=0;r<npermutaciones;r++){
    ...
    arma::uvec permaux=permutaciones.row(r).t();
    // Rcpp::Rcout << "The value is permutaciones " << permaux << std::endl;
    //
      //
      // Rcpp::Rcout << " Hola " << permaux << std::endl;
    // Rcpp::Rcout << " Holad " << permaux.elem(contador1) << std::endl;

    arma:: mat X2aux= Y.rows(permaux.elem(contador1));
    arma:: mat Y2aux= Y.rows(permaux.elem(contador2));

    // Rcpp::Rcout << " X2aux " << X2aux << std::endl;
    // Rcpp::Rcout << " X2aux " << Y2aux << std::endl;

    arma::mat d21 = (*kernel_Y).getKernelMatrix(X2aux,X2aux);
    arma::mat d22 = (*kernel_Y).getKernelMatrix(Y2aux,Y2aux);
    arma::mat d23 = (*kernel_Y).getKernelMatrix(X2aux,Y2aux);
    ...
    acum4=  2*acum4*(1/double(n2))*(1/double(n3));
    resultados(r)= acum*0.5+0.5*(acum2+acum3-acum4);
  }
